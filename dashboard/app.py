...
def load_data():
    ...
    try:
        ...
        with open("../api/data/call_logs.json", "r") as f:
            data = json.load(f)
...
# --- Main Dashboard ---
st.title("🚛 Acme Logistics - Inbound Carrier Sales Dashboard")
...
if df.empty:
    ...
    filtered_df = df[(df['call_timestamp'] >= start_datetime) & (df['call_timestamp'] <= end_datetime)]
    ...
    if filtered_df.empty:
        ...
        with col1:
            ...
            st.bar_chart(sentiment_df.set_index('Sentiment'), color=[sentiment_colors.get(s, '#1f77b4') for s in sentiment_df['Sentiment']])
